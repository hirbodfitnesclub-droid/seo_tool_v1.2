...
| کتابخانه خارجی برای محاسبات متنی | توابع Jaccard و شباهت عنوان باید دستی نوشته شوند |
| استفاده از Math.log بدون +1 برای IDF | همیشه `Math.log(total / (count + 1))` برای جلوگیری از division by zero |
...
- **الگوریتم امتیازدهی پیشرفته:**
  - IDF (وزن‌دهی بر اساس نادر بودن تگ)
  - Partial Match برای ماه‌ها و فصل‌های مجاور
  - Jaccard Bidirectional (محاسبه دوطرفه تطابق)
  - Title Text Similarity (شباهت متنی عنوان)
