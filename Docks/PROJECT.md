...
- خنثی (هیچ تطابقی — صفحه غیر زمان‌دار): `×1`
- خارج از فصل سبک (کلیدواژه فصلی بدون رویداد): `×0.15`
- **منقضی / بیش از حد دور (صفحه زمان‌دار با رویداد گذشته یا فاصله > ۶۰ روز): `×0.001` — Devastating Penalty**

> **اصلاحیه باگ Year-Wrap (خرداد ۱۴۰۴):** قبلاً پس از پایان رویدادِ امسال، سیستم رویداد را بلافاصله به سال بعد پرتاب می‌کرد و کاندیدا از جریمه فرار می‌کرد. حالا تا زمانی که `daysFromEnd` این‌سالی مثبت است و `daysToStart` سال بعد > ۶۰ روز است، کاندیدا «منقضی» تلقی می‌شود.
...
| **اعمال جریمه بر کاندیدای غیر زمان‌دار** | فقط کاندیداهایی که با کلیدواژه یک رویداد/فصل مطابقت دارند کاندید جریمه‌اند. صفحه عمومی «تور کیش» نباید چون رویدادی فعال نیست به ×0.15 یا ×0.001 برسد |
| **استفاده از `projectEventToCurrentYear` به‌صورت بی‌قید (Year-Wrap Bug)** | پرتاب رویداد به سال بعد فقط زمانی مجاز است که فاصله تا شروع سال بعد ≤ ۶۰ روز باشد. در بازه ۶۱+ روز، رویداد هنوز «گذشته» (Expired) است و باید ×0.001 بگیرد |
